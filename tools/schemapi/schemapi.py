...
def validate_jsonschema(
    ...
    *,
    ...
    if errors:
        leaf_errors = _get_leaves_of_error_tree(errors, rootschema or schema)
...
def _get_leaves_of_error_tree(
    errors: ValidationErrorList, root_schema: dict[str, Any]
) -> ValidationErrorList:
    """
        ...
        if err.context:
            ...
            leaves.extend(
                _get_leaves_of_error_tree(
                    _get_relevant_errors(err, root_schema), root_schema
                )
            )
        else:
            leaves.append(err)
...
def _root_union_branch(
    ...
    if (
        ...
        return schema_path[0], error.schema
    return None
...
def _decode_json_pointer_fragment(reference: str) -> str | None:
    ...
    try:
        ...
        return None
    return pointer if pointer == "" or pointer.startswith("/") else None


def _resolve_json_pointer(root_schema: dict[str, Any], reference: str) -> Any:
    pointer = _decode_json_pointer_fragment(reference)
    if pointer is None:
        return None
    if pointer == "":
        return root_schema
...
def _resolve_local_schema_reference(
    schema: Any, root_schema: dict[str, Any]
) -> dict[str, Any] | None:
    ...
    while isinstance(reference := current.get("$ref"), str):
        if reference.startswith(f"{_VEGA_LITE_ROOT_URI}#"):
            reference = reference.removeprefix(_VEGA_LITE_ROOT_URI)
        if (
        ...
        target = _resolve_json_pointer(root_schema, reference)
...
def _union_branch_schemas(
    errors: ValidationErrorList,
    union_schemas: list[Any],
    root_schema: dict[str, Any],
) -> dict[int, dict[str, Any] | None]:
    branch_schemas = {
        index: _resolve_local_schema_reference(schema, root_schema)
        for index, schema in enumerate(union_schemas)
    }
    for error in errors:
        branch = _root_union_branch(error)
        if branch is not None:
            index, schema = branch
            branch_schemas[index] = schema
    return branch_schemas
...
def _excluded_union_branch_indexes(
    ...
    for error in errors:
        ...
        if not schema_path or not isinstance(schema_path[0], int):
            continue
        required = (
            error.schema.get("required", []) if isinstance(error.schema, dict) else []
        )
        is_missing_root_requirement = (
            len(schema_path) == 2
            and error.validator == "required"
            and isinstance(required, list)
            and set(required).isdisjoint(instance)
        )
        is_direct_value_mismatch = (
            len(schema_path) == 4
...
def _mismatched_union_property_names(errors: ValidationErrorList) -> set[str]:
    names: set[str] = set()
    for error in errors:
        schema_path = list(error.schema_path)
        if (
            ...
            and isinstance(schema_path[0], int)
            and schema_path[1] == "properties"
            and isinstance(schema_path[2], str)
            and error.validator in {"const", "enum"}
        ):
            names.add(schema_path[2])
    return names


def _branch_accepts_property_values(
    schema: dict[str, Any] | None,
    property_names: set[str],
    instance: dict[str, Any],
) -> bool:
    if schema is None or not property_names:
        return False
    for name in property_names:
        property_schemas = _property_schemas(schema, name)
        if (
            name not in instance
            or property_schemas is None
            or any(
                _value_matches_schema(instance[name], property_schema) is not True
...
def _schema_contains_reference(schema: Any) -> bool:
    if isinstance(schema, dict):
        return "$ref" in schema or any(
            _schema_contains_reference(value) for value in schema.values()
        )
    if isinstance(schema, list):
        return any(_schema_contains_reference(value) for value in schema)
    return False


def _value_matches_schema(value: Any, schema: Any) -> bool | None:
...
def _property_schemas(schema: dict[str, Any], property_name: str) -> list[Any] | None:
    ...
    if property_name in properties:
        matched_schemas.append(properties[property_name])
    try:
        matched_schemas.extend(
            property_schema
            for pattern, property_schema in pattern_properties.items()
            if re.search(pattern, property_name)
        )
...
def _required_values_match_instance(
    required: Sequence[str], schema: dict[str, Any], instance: dict[str, Any]
) -> bool | None:
    has_unknown = False
    for name in required:
        property_schemas = _property_schemas(schema, name)
        if name not in instance or property_schemas is None:
            return False
        matches = [
            _value_matches_schema(instance[name], property_schema)
            for property_schema in property_schemas
        ]
        if any(match is False for match in matches):
            return False
        has_unknown = has_unknown or any(match is None for match in matches)
    return None if has_unknown else True


def _branch_has_required_value_error(
    errors: ValidationErrorList, branch_index: int, required: set[str]
) -> bool:
    return any(
        error.schema_path
        and error.schema_path[0] == branch_index
        and error.path
        and error.path[0] in required
        for error in errors
    )


def _branch_excludes_required_value(
    schema: dict[str, Any] | None,
    required: set[str],
    instance: dict[str, Any],
) -> bool:
...
def _required_values_identify_branch(
    required: set[str],
    branch_index: int,
...
def _matching_union_branch_indexes(
    error: jsonschema.exceptions.ValidationError,
    branch_schemas: dict[int, dict[str, Any] | None],
) -> set[int]:
    ...
    for branch_index, schema in branch_schemas.items():
        if schema is None:
            continue
        required = schema.get("required", [])
        required_names = set(required)
        if not required_names or not required_names.issubset(instance):
            continue
        required_values_match = _required_values_match_instance(
            required, schema, instance
        )
        if required_values_match is None:
        ...
        if required_values_match and _required_values_identify_branch(
            required_names, branch_index, branch_schemas, instance
        ):
            matching.add(branch_index)
    return matching


def _get_relevant_errors(
    error: jsonschema.exceptions.ValidationError, root_schema: dict[str, Any]
) -> ValidationErrorList:
    """Prefer union branches whose required properties identify the instance."""
    if (
        not isinstance(error.validator, str)
        or error.validator not in {"anyOf", "oneOf"}
        or not isinstance(error.instance, dict)
        or not isinstance(error.schema, dict)
    ):
        return error.context

    union_schemas = error.schema.get(error.validator, [])
    if not isinstance(union_schemas, list):
        return error.context

    branch_schemas = _union_branch_schemas(error.context, union_schemas, root_schema)
    ...
    if not matching_schema_indexes:
        ...
        if excluded_indexes and len(remaining_indexes) == 1:
            remaining_index = next(iter(remaining_indexes))
            if _branch_accepts_property_values(
                branch_schemas.get(remaining_index),
                mismatched_properties,
                error.instance,
            ):
                matching_schema_indexes = remaining_indexes
        elif excluded_indexes and remaining_indexes:
            matching_schema_indexes = remaining_indexes
