...
def validate_jsonschema(
    ...
    *,
    ...
    """
    errors, registry = _get_errors_from_spec(spec, schema, rootschema=rootschema)
    if errors:
        leaf_errors = _get_leaves_of_error_tree(
            errors, rootschema or schema, registry=registry
        )
...
def _get_errors_from_spec(
    ...
    schema: dict[str, Any],
    rootschema: dict[str, Any] | None = None,
) -> tuple[ValidationErrorList, Registry | None]:
    """
    ...
    if _use_referencing_library():
        ...
        registry = _get_referencing_registry(
        ...
        if err.context:
            ...
            leaves.extend(
                _get_leaves_of_error_tree(
                    _get_relevant_errors(err, root_schema, registry),
                    root_schema,
                    registry,
                )
            )
...
def _union_branch_schemas(
    ...
    branch_schemas = {
        index: (
            _resolve_references(schema, root_schema, registry=registry)
...
def _get_relevant_errors(
    ...
    branch_schemas = _union_branch_schemas(
        error.context, union_schemas, root_schema, registry
    )
...
def _resolve_references(
    ...
    *,
    ...
    if _use_referencing_library():
        registry = registry or _get_referencing_registry(rootschema or schema)
        ...
        while "$ref" in schema:
            ...
            if not reference.startswith(_VEGA_LITE_ROOT_URI):
                reference = _VEGA_LITE_ROOT_URI + reference
            resolved = referencing_resolver.lookup(reference)
            schema = resolved.contents
            referencing_resolver = resolved.resolver
    else:
