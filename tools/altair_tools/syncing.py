...
VERSION_FILE = """# Automatically written by tools/sync_vegalite_schema.py
# Do not modify this manually
vegalite_version = '{vegalite_version}'
vegalite_schema_url = '{vegalite_schema_url}'
"""

DATA_VERSION_FILE = """# Automatically written by tools/sync_vegalite_datasets.py
# Do not modify this manually
vega_data_version = '{vega_data_version}'
"""

EXAMPLE_VERSION_FILE = """# Automatically written by tools/sync_vegalite_examples.py
# Do not modify this manually
vegalite_version = '{vegalite_version}'
"""
...
def sync_examples(vegalite_version,
                  destination=path_within_altair('examples'),
    ...
    """
    json_destination = os.path.join(destination, 'json')
    
    if not os.path.exists(json_destination):
        os.makedirs(json_destination)

    # Write the version file
    with open(os.path.join(destination, version_file), 'w') as f:
        f.write(EXAMPLE_VERSION_FILE.format(vegalite_version=vegalite_version))
    ...
    index_destination = os.path.join(json_destination, '..',
                                     'example-listing.json')
    print("Reading from {0}".format(source_dir))

    if not os.path.exists(json_destination):
        raise ValueError("{0} does not exist".format(json_destination))
    ...
    with checkout_tag(vegalite_version, vegalite_path, vegalite_url):
        ...
        shutil.rmtree(json_destination)
        copy_tree_and_replace_urls(source_dir, json_destination)
...
def sync_datasets(datasets_version,
    ...
    # Write the version file
    with open(os.path.join(destination, version_file), 'w') as f:
        f.write(DATA_VERSION_FILE.format(vega_data_version=datasets_version))
