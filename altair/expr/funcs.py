...
FUNCTION_LISTING = {
  ...
  "indexof": ("returns the first index of an element (for array inputs) or substring (for string inputs) e.g., "
              "`indexof(\"visualization\", \"i\") == 1"),
  ...
  "open": ("opens a hyperlink (alias to `window.open`). This function is only valid when running in the browser. "
           "It should not be invoked within a server-side (e.g., node.js) environment."),
  ...
  "lastindexof": ("returns the last index of an element (for array inputs) or substring (for string inputs)  "
                  "e.g., `lastindexof(\"visualization\", \"i\") == 10"),
  "if": ("if the first argument evaluates true then the second argument is returned, otherwise the third argument is "
         "returned (`if(a, b, c)` is equivalent to `a ? b : c`)"),
  "timeFormat": ("formats a string as a local datetime; the first argument must be a valid "
                 "[d3-time-format specifier](https://github.com/d3/d3-time-format/) "
                 "(e.g., `timeFormat(\"%A\", datum.timestamp)`)"),
  ...
  "test": ("evaluates a regular expression against a string, returning true if the string matches the pattern, "
           "false otherwise. _Example_: `test(/\\d{3}/, \"32-21-9483\") -> true"),
  "utcmonth": "returns the (zero-based) month for a given date input, in UTC time",
  "eventGroup": ("returns the scenegraph group mark item within which the current event has occurred. "
                 "If no arguments are provided, the immediate parent group is returned. If a group name is provided, "
                 "the matching ancestor group item is returned."),
  ...
  "indata": ("tests if a specified datasource contains a tuple with a given value for a specific field "
             "(i.e., `indata(\"table\", val, \"price\")`)"),
  ...
  "eventX": ("returns the x-coordinate for the current event. If no arguments are provided, the top-level "
             "coordinate space of the visualization is used. If a group name is provided, the coordinate-space "
             "of the matching ancestor group item is used."),
  "month": "returns the (zero-based) month for a given date input, in local time",
  "sin": "trigonometric sine (alias to `Math.sin`)",
  "datetime": ("returns a new `Date` instance _(year, month[, day, hour, min, sec, millisec])_  Note that, "
               "just like Javascript, `month` is 0-based. For example, `1` represents February."),
  "scale": ("applies a named scale transform to a specified value; by default, looks for the scale at the "
            "top-level of the specification, but an optional signal can also be supplied corresponding to the "
            "group which contains the scale (i.e., `scale(\"x\", val, group)`). *Note:* This function is "
            "only legal within signal stream handlers and mark "
            "[production rules](https://github.com/vega/vega/wiki/Marks#production-rules). Invoking this function "
            "elsewhere (e.g., with filter or formula transforms) will result in an error."),
  ...
  "utcFormat": ("formats a string as a [UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time) datetime; "
                "the first argument must be a valid "
                "[d3-time-format specifier](https://github.com/mbostock/d3/wiki/Time-Formatting) "
                "(e.g., `utcFormat(\"%\", datum.timestamp)`)."),
  ...
  "format": ("formats a string as a numeric value; the first argument must be a valid "
             "[d3-format specifier](https://github.com/d3/d3-format/) (e.g., `format(\",.2f\", datum.value)`)"),
  ...
  "iscale": ("applies an inverse scale transform to a specified value; by default, looks for the scale at the "
             "top-level of the specification, but an optional signal can also be supplied corresponding to the group "
             "which contains the scale (i.e., `iscale(\"x\", val, group)`). *Note:* This function is only legal "
             "within signal stream handlers and mark "
             "[production rules](https://github.com/vega/vega/wiki/Marks#production-rules). Invoking this function "
             "elsewhere (e.g., with filter or formula transforms) will result in an error."),
  ...
  "inrange": ("tests whether a value falls within a specified inclusive extent; an optional flag uses an exclusive "
              "extent instead (i.e., `inrange(value, a, b, exclusive?) `)."),
  "eventY": ("returns the y-coordinate for the current event. If no arguments are provided, the top-level "
             "coordinate space of the visualization is used. If a group name is provided, the coordinate-space "
             "of the matching ancestor group item is used.")
}
