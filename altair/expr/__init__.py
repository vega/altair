...
class expr(_ExprRef, metaclass=_ConstExpressionType):
    ...
    `Expressions`_ can used to write basic formulas that enable custom interactions.
    ...
    >>> param_color = alt.param(expr=alt.expr.if_(param_width < 200, 'red', 'black'))
    ...
    def week(cls, *args) -> FunctionExpression:
        ...
        Days before the first Sunday of the year are considered to be in week 0, the first Sunday of the year is the start of week 1, the second Sunday week 2, _etc._."""
    ...
    def clampRange(cls, *args) -> FunctionExpression:
        ...
        If the span of the input *range* is less than _(max - min)_ and an endpoint exceeds either the *min* or *max* value, the range is translated such that the span is preserved and one endpoint touches the boundary of the _[min, max]_ range.
        If the span exceeds _(max - min)_, the range _[min, max]_ is returned."""
    ...
    def sequence(cls, *args) -> FunctionExpression:
        ...
        If *step* is positive, the last element is the largest _start + i * step_ less than _stop_; if *step* is negative, the last element is the smallest _start + i * step_ greater than *stop*.
    ...
    def slice(cls, *args) -> FunctionExpression:
        ...
        If the *end* argument is negative, it is treated as an offset from the end of the array (_length(array) + end_)."""
    ...
    def truncate(cls, *args) -> FunctionExpression:
        r"""Truncates an input *string* to a target *length*.
        ...
        The optional *ellipsis* argument indicates the string to use to indicate truncated content; by default the ellipsis character `...` (`\\u2026`) is used."""
    ...
    def geoBounds(cls, *args) -> FunctionExpression:
        ...
        The bounding box is represented by a two-dimensional array: [[_x0_, _y0_], [_x1_, _y1_]], where *x0* is the minimum x-coordinate, *y0* is the minimum y-coordinate, *x1* is the maximum x-coordinate, and *y1* is the maximum y-coordinate.
